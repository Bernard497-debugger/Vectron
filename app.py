...
def admin_page():
    ...
    pending_pay = sum(1 for p in all_pend.values() if p.get("status") == "pending")
    ...
    # Build pending rows
    pend_rows = ""
    for pid, pay in sorted(all_pend.items(), key=lambda x: x[1].get("created_at", ""), reverse=True):
        ...
        pend_rows += f"""<tr style="border-bottom:1px solid rgba(255,255,255,0.05)">
          ...
          <td style="padding:9px 8px;font-size:11px">{str(pay.get('created_at',''))[:16]}</td>
    ...
    if not pend_rows:
        pend_rows = '<tr><td colspan="7" style="padding:24px;color:#5a6a85;text-align:center">No pending payments</td></tr>'
...
<div class="stats">
  ...
  <div class="stat"><div class="stat-val warn">{{ pending_pay }}</div><div class="stat-label">Pending Payments</div></div>
...
<div class="tab-bar">
  ...
  <button class="tab-btn" onclick="switchTab('pending', this)">⏳ Pending Payments{% if pending_pay > 0 %} <span style="background:#f06a4d;color:#fff;font-size:10px;padding:1px 5px;border-radius:100px">{{ pending_pay }}</span>{% endif %}</button>
...
<div id="tab-pending" class="tab-content card">
  ...
  <div style="font-size:13px;color:#8899b4;margin-bottom:14px;line-height:1.6">
    These users uploaded a receipt but the subscription is NOT yet activated.
    Approve once you've confirmed Orange Money payment. Reject if invalid.
  </div>
  <div class="overflow"><table style="{{ TABLE }}">
    <tr>
      <th style="{{ TH }}">Payment ID</th><th style="{{ TH }}">Plan</th><th style="{{ TH }}">Amount</th>
      <th style="{{ TH }}">Phone</th><th style="{{ TH }}">Country</th>
      <th style="{{ TH }}">Submitted</th><th style="{{ TH }}">Actions</th>
    </tr>
    {{ pend_rows|safe }}
  </table></div>
</div>
...
async function approvePayment(pid) {
  ...
  if(j.success) { toast('✓ Payment approved, plan activated'); setTimeout(()=>location.reload(),1200); }
...
""", active_pro=active_pro, active_ent=active_ent, pending_pay=pending_pay,
...
def admin_stats():
    ...
    return jsonify({
        ...
        "pending_payments": sum(1 for p in all_pend.values() if p.get("status") == "pending"),
