...
from flask import Flask, request, jsonify, session, send_file, render_template_string, redirect
...
ADMIN_PASSWORD = os.environ.get("ADMIN_PASSWORD", "vectron_admin_2026")
...
def _load(name):
    ...
    try:
        with open(path, "r") as f:
            return _json_store.load(f)
    except Exception:
        return {}

def _save(name, data):
    with _store_lock:
        with open(_store_path(name), "w") as f:
            _json_store.dump(data, f, indent=2, default=str)
...
PENDING_PAYMENTS = _PersistentDict("pending_payments")
VERIFIED_PAYMENTS = _PersistentDict("verified_payments")
SUBSCRIPTIONS = _PersistentDict("subscriptions")
...
def upload_receipt():
    ...
    # Update payment status to pending_approval (NOT verified yet)
    ...
    return jsonify({
        "success": True,
        "message": "✅ Receipt uploaded! Waiting for admin approval. You will be notified when your account is upgraded."
    })
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  body{background:#0a0a12;color:#e8f0ff;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;
       display:flex;align-items:center;justify-content:center;min-height:100vh}
  .box{background:#0f1117;border:1px solid rgba(255,255,255,0.08);border-radius:18px;
       padding:40px 36px;width:100%;max-width:360px;box-shadow:0 30px 80px rgba(0,0,0,0.7)}
  h1{font-size:22px;font-weight:800;color:#7C3AED;margin-bottom:6px}
  .sub{font-size:13px;color:#5a6a85;margin-bottom:28px}
  label{font-size:12px;color:#8899b4;display:block;margin-bottom:6px;text-transform:uppercase;letter-spacing:0.4px}
  input{width:100%;background:#070710;border:1px solid rgba(255,255,255,0.1);border-radius:10px;
        padding:11px 14px;color:#e8f0ff;font-size:14px;outline:none;margin-bottom:16px}
  ...
  .err{color:#f06a4d;font-size:13px;margin-bottom:12px}
</style></head><body>
<div class="box">
  <h1>⚡ Vectron</h1>
...
def admin_page():
    if not session.get("admin_authed"):
        ...
        if not (ADMIN_KEY and key == ADMIN_KEY):
            return redirect("/admin/login")
        session["admin_authed"] = True

    all_subs = dict(SUBSCRIPTIONS)
    all_pay = dict(VERIFIED_PAYMENTS)
    all_pend = dict(PENDING_PAYMENTS)

    active_pro = sum(1 for s in all_subs.values() if _sub_is_active(s) and s.get("plan") == "pro")
    active_ent = sum(1 for s in all_subs.values() if _sub_is_active(s) and s.get("plan") == "enterprise")
    total_rev = sum(float(p.get("amount_usd", 0)) for p in all_pay.values())
    pending_approvals = sum(1 for p in all_pend.values() if p.get("status") == "pending_approval")
    total_subs = len(all_subs)

    # Build subscription rows
    sub_rows = ""
    PLAN_COLORS = {"pro": "#4DF0C0", "enterprise": "#7B6EF6", "free": "#5a6a85"}
    for uid, sub in sorted(all_subs.items(), key=lambda x: x[1].get("started_at", ""), reverse=True):
        is_active = _sub_is_active(sub)
        plan = sub.get("plan", "free")
        col = PLAN_COLORS.get(plan, "#5a6a85")
        status_c = "#4DF0C0" if is_active else "#f06a4d"
        exp = str(sub.get("expires_at", ""))[:19] or "Never"
        country = sub.get("country", "—")
        receipt = sub.get("receipt_filename", "")
        started = str(sub.get("started_at", ""))[:16]
        sub_rows += f"""<tr style="border-bottom:1px solid rgba(255,255,255,0.05)">
          <td style="padding:9px 8px;font-size:11px;color:#8899b4;max-width:140px;overflow:hidden;text-overflow:ellipsis">{uid[:20]}…</td>
          <td style="padding:9px 8px;color:{col};font-weight:700;text-transform:uppercase">{plan}</td>
          <td style="padding:9px 8px;color:{status_c};font-weight:700">{"● Active" if is_active else "○ Expired"}</td>
          <td style="padding:9px 8px;font-size:11px">{exp}</td>
          <td style="padding:9px 8px;font-size:11px">{country}</td>
          <td style="padding:9px 8px;font-size:11px">{started}</td>
          <td style="padding:9px 8px">{"<a href='/admin/receipt/"+receipt+"' target='_blank' style='color:#4DF0C0;font-size:11px'>View receipt</a>" if receipt else "—"}</td>
          <td style="padding:9px 8px">
            <button onclick="grantPlan('{uid}')" style="background:rgba(124,58,237,0.2);border:1px solid #7C3AED;color:#a78bfa;padding:4px 12px;border-radius:6px;cursor:pointer;font-size:11px;">Edit</button>
            <button onclick="revokeSub('{uid}')" style="background:rgba(240,106,77,0.15);border:1px solid #f06a4d;color:#f06a4d;padding:4px 12px;border-radius:6px;cursor:pointer;font-size:11px;margin-left:4px">Revoke</button>
          </td>
        </tr>"""

    if not sub_rows:
        sub_rows = '<tr><td colspan="8" style="padding:24px;color:#5a6a85;text-align:center">No subscriptions yet</td></tr>'

    # Build pending approval rows
    pend_rows = ""
    for pid, pay in sorted(all_pend.items(), key=lambda x: x[1].get("submitted_at", x[1].get("created_at", "")), reverse=True):
        if pay.get("status") != "pending_approval":
            continue
        plan_name = SUBSCRIPTION_PLANS.get(pay.get("plan_id", ""), {}).get("name", "?")
        submitted_at = pay.get("submitted_at", pay.get("created_at", ""))
        pend_rows += f"""<tr style="border-bottom:1px solid rgba(255,255,255,0.05)">
          <td style="padding:9px 8px;font-size:11px">{pid[:14]}…</td>
          <td style="padding:9px 8px">{plan_name}</td>
          <td style="padding:9px 8px">{pay.get('amount_local',0)} {pay.get('currency','')}</td>
          <td style="padding:9px 8px;font-size:11px">{pay.get('phone_number','—')}</td>
          <td style="padding:9px 8px;font-size:11px">{pay.get('country','—')}</td>
          <td style="padding:9px 8px;font-size:11px">{str(submitted_at)[:16]}</td>
          <td style="padding:9px 8px">
            {"<a href='/admin/receipt/"+pay.get('receipt_filename','')+"' target='_blank' style='color:#4DF0C0;font-size:11px'>View Receipt</a>" if pay.get('receipt_filename') else "—"}
          </td>
          <td style="padding:9px 8px;display:flex;gap:6px">
            <button onclick="approvePayment('{pid}')" style="background:rgba(77,240,192,0.15);border:1px solid #4DF0C0;color:#4DF0C0;padding:4px 12px;border-radius:6px;cursor:pointer;font-size:11px;">✓ Approve</button>
            <button onclick="rejectPayment('{pid}')" style="background:rgba(240,106,77,0.15);border:1px solid #f06a4d;color:#f06a4d;padding:4px 12px;border-radius:6px;cursor:pointer;font-size:11px;">✕ Reject</button>
          </td>
        </tr>"""

    if not pend_rows:
        pend_rows = '<tr><td colspan="8" style="padding:24px;color:#5a6a85;text-align:center">No pending approvals</td></tr>'

    # Build verified payment rows
    ver_rows = ""
    for pid, pay in sorted(all_pay.items(), key=lambda x: x[1].get("verified_at", ""), reverse=True):
        plan_name = SUBSCRIPTION_PLANS.get(pay.get("plan_id", ""), {}).get("name", "?")
        receipt = pay.get("receipt_filename", "")
        ver_rows += f"""<tr style="border-bottom:1px solid rgba(255,255,255,0.05)">
          <td style="padding:9px 8px;font-size:11px">{pid[:14]}…</td>
          <td style="padding:9px 8px">{plan_name}</td>
          <td style="padding:9px 8px">${pay.get('amount_usd',0)}</td>
          <td style="padding:9px 8px;font-size:11px">{pay.get('phone_number','—')}</td>
          <td style="padding:9px 8px;font-size:11px">{pay.get('country','—')}</td>
          <td style="padding:9px 8px;font-size:11px">{str(pay.get('verified_at',''))[:16]}</td>
          <td style="padding:9px 8px">{"<a href='/admin/receipt/"+receipt+"' target='_blank' style='color:#4DF0C0;font-size:11px'>View</a>" if receipt else "—"}</td>
        </tr>"""

    if not ver_rows:
        ver_rows = '<tr><td colspan="7" style="padding:24px;color:#5a6a85;text-align:center">No verified payments yet</td></tr>'

    TH = "padding:10px 8px;text-align:left;color:#7C3AED;font-size:11px;border-bottom:1px solid rgba(124,58,237,0.2);text-transform:uppercase;letter-spacing:0.4px"
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  body{background:#0a0a12;color:#e8f0ff;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;padding:24px}
  h1{color:#7C3AED;font-size:22px;font-weight:800;margin-bottom:4px}
  .sub{font-size:13px;color:#5a6a85;margin-bottom:26px}
  .stats{display:flex;gap:12px;flex-wrap:wrap;margin-bottom:26px}
  .stat{background:#0f1117;border:1px solid rgba(124,58,237,0.2);border-radius:12px;padding:16px 20px;min-width:120px}
  .stat-val{font-size:26px;font-weight:800;color:#7C3AED}
  .stat-val.green{color:#4DF0C0}.stat-val.warn{color:#F0C44D}
  .stat-label{font-size:11px;color:#5a6a85;margin-top:4px;text-transform:uppercase;letter-spacing:0.4px}
  .tab-bar{display:flex;gap:6px;margin-bottom:20px;flex-wrap:wrap}
  .tab-btn{background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);color:#8899b4;
           padding:8px 16px;border-radius:8px;font-size:13px;font-weight:600;cursor:pointer;transition:all 0.2s}
  .tab-btn.active{background:rgba(124,58,237,0.15);border-color:rgba(124,58,237,0.4);color:#a78bfa}
  .tab-content{display:none}.tab-content.active{display:block}
  .card{background:#0f1117;border:1px solid rgba(255,255,255,0.07);border-radius:14px;padding:22px;margin-bottom:24px}
  .section-title{font-size:15px;font-weight:700;color:#e8f0ff;margin-bottom:16px}
  .overflow{overflow-x:auto}
  .form-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:12px;margin-bottom:14px}
  label{font-size:12px;color:#8899b4;display:block;margin-bottom:5px;text-transform:uppercase;letter-spacing:0.3px}
  input,select{width:100%;background:#070710;border:1px solid rgba(255,255,255,0.1);border-radius:8px;
               padding:9px 12px;color:#e8f0ff;font-size:13px;outline:none;transition:border-color 0.2s}
  input:focus,select:focus{border-color:rgba(124,58,237,0.5)}
  .toast{position:fixed;bottom:24px;right:24px;background:#0f1117;border:1px solid rgba(124,58,237,0.4);
         color:#a78bfa;padding:12px 20px;border-radius:10px;font-size:13px;z-index:9999;display:none}
...
<div class="sub">
  ...
  <form method="post" action="/admin/logout" style="display:inline">
    <button style="background:none;border:none;color:#f06a4d;cursor:pointer;font-size:13px">Sign out</button>
  </form>
</div>

<div class="stats">
  <div class="stat"><div class="stat-val green">{{ active_pro }}</div><div class="stat-label">Pro Subs</div></div>
  <div class="stat"><div class="stat-val" style="color:#7B6EF6">{{ active_ent }}</div><div class="stat-label">Enterprise</div></div>
  <div class="stat"><div class="stat-val warn">{{ pending_approvals }}</div><div class="stat-label">Pending Approvals</div></div>
  <div class="stat"><div class="stat-val">${{ "%.2f"|format(total_rev) }}</div><div class="stat-label">Total Revenue</div></div>
  <div class="stat"><div class="stat-val">{{ total_subs }}</div><div class="stat-label">Total Subs</div></div>
</div>

<div class="tab-bar">
  <button class="tab-btn active" onclick="switchTab('subs', this)">💎 Subscriptions</button>
  <button class="tab-btn" onclick="switchTab('pending', this)">⏳ Pending Approvals{% if pending_approvals > 0 %} <span style="background:#f06a4d;color:#fff;font-size:10px;padding:1px 5px;border-radius:100px">{{ pending_approvals }}</span>{% endif %}</button>
  <button class="tab-btn" onclick="switchTab('verified', this)">✅ Verified Payments</button>
  <button class="tab-btn" onclick="switchTab('grant', this)">➕ Grant Plan</button>
</div>

<!-- Subscriptions -->
<div id="tab-subs" class="tab-content active card">
  <div class="section-title">All Subscriptions</div>
  <div class="overflow"><table style="{{ TABLE }}">
    <tr>
      <th style="{{ TH }}">User ID</th><th style="{{ TH }}">Plan</th><th style="{{ TH }}">Status</th>
      <th style="{{ TH }}">Expires</th><th style="{{ TH }}">Country</th><th style="{{ TH }}">Started</th>
      <th style="{{ TH }}">Receipt</th><th style="{{ TH }}">Actions</th>
    </tr>
    {{ sub_rows|safe }}
  </table></div>
</div>
...
<!-- Pending approvals -->
<div id="tab-pending" class="tab-content card">
  <div class="section-title">⏳ Pending Payment Approvals</div>
  <div style="font-size:13px;color:#8899b4;margin-bottom:14px;line-height:1.6">
    These users uploaded a receipt and are waiting for admin approval.
    Verify the Orange Money payment before approving.
  </div>
  <div class="overflow"><table style="{{ TABLE }}">
    <tr>
      <th style="{{ TH }}">Payment ID</th><th style="{{ TH }}">Plan</th><th style="{{ TH }}">Amount</th>
      <th style="{{ TH }}">Phone</th><th style="{{ TH }}">Country</th>
      <th style="{{ TH }}">Submitted</th><th style="{{ TH }}">Receipt</th><th style="{{ TH }}">Actions</th>
    </tr>
    {{ pend_rows|safe }}
  </table></div>
</div>
...
<!-- Verified payments -->
<div id="tab-verified" class="tab-content card">
  <div class="section-title">✅ Verified Payments</div>
  <div class="overflow"><table style="{{ TABLE }}">
    <tr>
      <th style="{{ TH }}">Payment ID</th><th style="{{ TH }}">Plan</th><th style="{{ TH }}">USD</th>
      <th style="{{ TH }}">Phone</th><th style="{{ TH }}">Country</th>
      <th style="{{ TH }}">Verified At</th><th style="{{ TH }}">Receipt</th>
    </tr>
    {{ ver_rows|safe }}
  </table></div>
</div>
...
<!-- Grant plan -->
<div id="tab-grant" class="tab-content card">
  <div class="section-title">➕ Manually Grant a Plan</div>
  <div style="background:rgba(124,58,237,0.06);border:1px solid rgba(124,58,237,0.15);border-radius:12px;padding:18px">
    <div class="form-grid">
      <div>
        <label>User ID (session ID)</label>
        <input type="text" id="grant-uid" placeholder="paste user_id here">
      </div>
      <div>
        <label>Plan</label>
        <select id="grant-plan">
          ...
          <option value="enterprise">Enterprise ($49.99/mo)</option>
        </select>
      </div>
      <div>
        <label>Days (0 = 30)</label>
        <input type="number" id="grant-days" value="30" min="1">
      </div>
    </div>
    <button onclick="manualGrant()" style="background:#7C3AED;color:#fff;border:none;padding:10px 20px;border-radius:8px;font-size:13px;font-weight:700;cursor:pointer">💎 Grant Plan</button>
    <span id="grant-msg" style="margin-left:12px;font-size:13px"></span>
  </div>
</div>
...
function toast(msg, ok=true) {
  ...
  t.style.borderColor = ok ? 'rgba(124,58,237,0.5)' : 'rgba(240,106,77,0.5)';
  t.style.color = ok ? '#a78bfa' : '#f06a4d';
  setTimeout(() => t.style.display = 'none', 3500);
}

async function approvePayment(pid) {
  const res = await fetch('/admin/api/payment/' + pid + '/approve', {method:'POST'});
  const j = await res.json();
  if(j.success) { toast('✓ Payment approved, subscription activated'); setTimeout(()=>location.reload(),1200); }
  else toast('Error: ' + (j.error||'?'), false);
}

async function rejectPayment(pid) {
  if(!confirm('Reject this payment?')) return;
  const res = await fetch('/admin/api/payment/' + pid + '/reject', {method:'POST'});
  const j = await res.json();
  if(j.success) { toast('✕ Payment rejected'); setTimeout(()=>location.reload(),1200); }
  else toast('Error: ' + (j.error||'?'), false);
}

async function grantPlan(uid) {
  const plan = document.getElementById('grant-plan').value;
  const days = parseInt(document.getElementById('grant-days').value) || 30;
  const uidInput = document.getElementById('grant-uid');
  const targetUid = uid || uidInput.value.trim();
  if(!targetUid) { toast('Please enter a User ID', false); return; }
  
  const res = await fetch('/admin/api/grant', {
    method:'POST',
    headers:{'Content-Type':'application/json'},
    body:JSON.stringify({user_id:targetUid, plan, days})
  });
  const j = await res.json();
  if(j.success) { toast('✓ ' + j.message); setTimeout(()=>location.reload(),1200); }
  else toast('Error: ' + (j.error||'?'), false);
}

async function revokeSub(uid) {
  if(!confirm('Revoke subscription for ' + uid.slice(0,20) + '...?')) return;
...
async function manualGrant() { grantPlan(null); }
...
window.manualGrant = manualGrant;
window.switchTab = switchTab;
</script>
</body></html>
""", active_pro=active_pro, active_ent=active_ent, pending_approvals=pending_approvals,
total_rev=total_rev, total_subs=total_subs, sub_rows=sub_rows,
pend_rows=pend_rows, ver_rows=ver_rows, TH=TH, TABLE=TABLE)

# ─── ADMIN API ROUTES ────────────────────────────────────────────────────────

@app.route("/admin/api/payment/<payment_id>/approve", methods=["POST"])
@admin_required
def admin_approve_payment(payment_id):
    ...
    if payment.get("status") != "pending_approval":
        return jsonify({"error": "Payment is not pending approval"}), 400

    user_id = payment.get("user_id")
    plan_id = payment.get("plan_id")
    if not user_id or not plan_id:
        return jsonify({"error": "Missing user or plan"}), 400
    ...
    return jsonify({"success": True, "message": "Payment approved and subscription activated"})

@app.route("/admin/api/payment/<payment_id>/reject", methods=["POST"])
@admin_required
...
def admin_grant_plan():
    ...
    if plan == "free":
        ...
        return jsonify({"success": True, "message": f"Subscription revoked for {user_id[:12]}..."})
    ...
    return jsonify({"success": True, "message": f"{plan.upper()} granted for {days} days to {user_id[:12]}..."})
...
def admin_revoke_plan():
    ...
    if not user_id:
        return jsonify({"error": "User ID required"}), 400
    SUBSCRIPTIONS.pop(user_id, None)
    return jsonify({"success": True, "message": f"Revoked for {user_id[:12]}..."})
...
def admin_stats():
    ...
    return jsonify({
        ...
        "active_subscriptions": sum(1 for s in all_subs.values() if _sub_is_active(s)),
        "plans": {p: sum(1 for s in all_subs.values() if s.get("plan") == p) for p in ["free", "pro", "enterprise"]},
        "total_payments": len(all_pend) + len(all_pay),
        "verified_payments": len(all_pay),
        "pending_approvals": sum(1 for p in all_pend.values() if p.get("status") == "pending_approval"),
        "total_revenue": sum(float(p.get("amount_usd", 0)) for p in all_pay.values())
    })
...
<div class="modal-overlay" id="upgrade-modal">
  <div class="modal">
    ...
    <p style="font-size:10px;color:var(--text-dim);margin-top:12px;text-align:center;">🔒 Secure · Upload receipt for admin approval</p>
  </div>
</div>
  ...
  if(paymentStep==='payment'){
    ...
    try{
      ...
      if(data.success){
        ...
        paymentStep='receipt';btn.textContent='Upload Receipt & Wait for Approval';btn.disabled=false;
  ...
  if(paymentStep==='receipt'){
    ...
    try{
      ...
      if(data.success){
        alert(data.message);
        closeUpgradeModal();
        document.getElementById('remaining-badge').textContent='⏳ Your upgrade is pending admin approval. You will be notified when approved.';
      } else {
        alert('Failed: '+(data.error||'Try again'));
        btn.disabled=false;
        btn.textContent='Upload Receipt & Wait for Approval';
      }
    }catch(e){alert('Error: '+e.message);btn.disabled=false;btn.textContent='Upload Receipt & Wait for Approval';}
  }
}
