...
# Configure upload folder for receipts and videos
...
SUBSCRIPTION_PLANS = {
    "free": {
        ...
        "features": ["Basic AI Chat", "Code Generation", "Calculator Tool", "System Info", "Image Generation (5/day)", "Video Generation (1/day)"],
    ...
    "pro": {
        ...
        "features": ["Everything in Free", "Priority Response", "Longer Context (128k)", "Advanced Models", "More Images (50/day)", "More Videos (10/day)", "No Ads"],
        "models": ["laguna-llama-3.2-3b-instruct:free"]
    },
    "enterprise": {
        "name": "Enterprise",
...
# Orange Money phone number prefix for different countries
...
- Generate videos when asked (use the video generation tool)
...
def check_rate_limit():
    ...
    video_limit = 1 if plan == "free" else 10 if plan == "pro" else 100
    
    if usage.get("images", 0) >= image_limit:
        return False, f"You've reached your daily image limit ({image_limit} images). Please upgrade to Pro for just $12/month!"
    
    if usage.get("videos", 0) >= video_limit:
        return False, f"You've reached your daily video limit ({video_limit} videos). Please upgrade to Pro for just $12/month!"
...
def update_usage(messages_count=1, tokens_used=0, images_count=0, videos_count=0):
    today = datetime.now().date().isoformat()
    if "usage" not in session:
        session["usage"] = {}
    if today not in session["usage"]:
...
def generate_video_wan26(prompt, duration=5, resolution="512x512"):
    """Generate a video using Alibaba Wan-2.6 model via OpenRouter"""
    if not OPENROUTER_API_KEY:
        raise ValueError("OPENROUTER_API_KEY not set")
    ...
    try:
        print(f"Generating video with Wan-2.6: {prompt}")
        ...
        return {
            "success": False,
            "message": "No video URL returned from the model. Please try again.",
            "prompt": prompt
        }
        ...
        if e.response and e.response.status_code == 402:
            return {
                "success": False,
                "message": "Video generation requires API credits. Please upgrade to Pro plan for video generation access.",
                "requires_upgrade": True
            }
        else:
            return {
                "success": False,
                "message": f"Video generation error: {error_detail}",
...
def initiate_payment():
    ...
    return jsonify({
        ...
        "message": f"Please send {amount_local} {currency} via {country_info['mobile_money']} to complete your subscription."
    })
...
def generate_video_route():
    """Generate a video using Alibaba Wan-2.6"""
    data = request.json or {}
    prompt = data.get("prompt", "").strip()
    ...
    try:
        video_result = generate_video_wan26(prompt, duration, resolution)
        ...
        if video_result.get("success"):
            ...
            plan = get_user_plan()
            video_limit = 1 if plan == "free" else 10 if plan == "pro" else 100
            remaining_videos = video_limit - usage.get("videos", 0)
            
            return jsonify({
                "success": True,
                "video_url": video_result.get("video_url"),
                "prompt": prompt,
                "model": video_result.get("model", "Alibaba Wan-2.6"),
                "remaining_videos": remaining_videos
            })
        else:
            return jsonify({
                "success": False,
                "error": video_result.get("message", "Video generation failed"),
                "requires_upgrade": video_result.get("requires_upgrade", False),
                "prompt": prompt
            })
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def tool():
    ...
    if tool_name == "calculate":
        ...
        try:
            video_result = generate_video_wan26(args)
            if video_result.get("success"):
                update_usage(videos_count=1)
                return jsonify({
                    "result": f"🎬 **Video Generated with Alibaba Wan-2.6!**\n\n**Prompt:** {args}\n\n<video controls width='100%'><source src='{video_result.get('video_url')}' type='video/mp4'></video>\n\n[Click here to download]({video_result.get('video_url')})",
                    "video_url": video_result.get("video_url"),
                    "type": "video"
                })
            else:
                return jsonify({
                    "result": video_result.get("message", "Video generation failed. Please try again."),
                    "type": "text"
                })
        except Exception as e:
            return jsonify({"result": f"Error generating video: {e}"}), 500
...
<div class="sheet" id="sheet">
  ...
  <div id="sheet-tools-section">
    ...
    <div class="sheet-tools">
      ...
      <button class="sheet-tool-btn" onclick="sheetTool('video')">🎬 Generate Video</button>
    </div>
  </div>
...
<div class="main">
  ...
  <aside class="sidebar">
    ...
    <button class="tool-btn" onclick="openTool('video')"><div class="tool-icon">🎬</div> Generate Video</button>
    <div class="sidebar-divider"></div>
  ...
  <div class="chat-area">
    <div id="messages">
      <div class="empty-state" id="empty-state">
        ...
        <p>Ask anything, generate code, create images, generate videos, or use the tools.</p>
        <div class="suggestion-chips">
          ...
          <div class="chip" onclick="sendSuggestion('Generate a video of a drone flying over mountains')">🎬 Mountain Drone</div>
        </div>
      </div>
    </div>
...
function appendToolResult(name, result, saved, type, extra) {
  ...
  if (type === 'image' && extra && extra.image_url) {
    contentHtml = `<div class="image-container"><img src="${extra.image_url}" alt="Generated Image" class="generated-image" onerror="this.src='https://placehold.co/512x512?text=Image+Failed+to+Load'"></div>`;
    if (extra.prompt) {
      contentHtml += `<div style="margin-top:8px;font-size:12px;color:var(--text-muted);">Prompt: ${escapeHtml(extra.prompt)}</div>`;
    }
    if (extra.remaining !== undefined) {
      contentHtml += `<div style="margin-top:8px;font-size:11px;color:var(--orange);">🎨 ${extra.remaining} images remaining today</div>`;
    }
  } else if (type === 'video' && extra && extra.video_url) {
    contentHtml = `
      <div class="video-container">
        <video controls class="generated-video" width="100%">
          <source src="${extra.video_url}" type="video/mp4">
          Your browser does not support video.
        </video>
        <div class="video-info">
          <span>🎬 Alibaba Wan-2.6</span>
          <span>⏱️ ~5 seconds</span>
          <a href="${extra.video_url}" download style="color:var(--accent);">📥 Download</a>
        </div>
      </div>`;
    if (extra.prompt) {
      contentHtml += `<div style="margin-top:8px;font-size:12px;color:var(--text-muted);">Prompt: ${escapeHtml(extra.prompt)}</div>`;
    ...
    if (extra.remaining !== undefined) {
      contentHtml += `<div style="margin-top:8px;font-size:11px;color:var(--accent);">🎬 ${extra.remaining} videos remaining today</div>`;
    }
  } else {
...
const toolConfig={
  ...
  video:{title:'Generate Video (Alibaba Wan-2.6)',desc:'Describe the video you want to create.',placeholder:'e.g. A drone flying over a futuristic city at sunset, cinematic 4K'}
};
...
async function runTool(){
  ...
  if(tool === 'image') {
    ...
    try {
      ...
      if(data.error) {
        ...
        appendToolResult('Generated Image', '', null, 'image', {
          image_url: data.image_url,
          prompt: data.prompt,
          remaining: data.remaining_images
        });
  ...
  if(tool === 'video') {
    if(!args) {
      appendToolResult('Generate Video', 'Please describe the video you want to create.', null, 'text');
      setLoading(false);
      return;
    }
    ...
    try {
      const res = await fetch('/generate-video', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({prompt: args, duration: 5, resolution: "512x512"})
      });
      const data = await res.json();
      removeThinking();
      ...
      if(data.error) {
        if(res.status === 429) {
          ...
          appendToolResult('Video Generation', data.error, null, 'text');
          openUpgradeModal();
        } else {
          appendToolResult('Error', data.error, null, 'text');
        }
      } else if(data.success) {
        appendToolResult('Generated Video', '', null, 'video', {
          video_url: data.video_url,
          prompt: data.prompt,
          remaining: data.remaining_videos
        });
        
        if(data.remaining_videos !== undefined) {
          document.getElementById('remaining-badge').textContent = `✨ ${data.remaining_videos} videos remaining today`;
        }
      } else {
        appendToolResult('Video Generation', data.error || 'Failed to generate video', null, 'text');
      }
    } catch(e) {
      removeThinking();
      appendToolResult('Error', e.message, null, 'text');
...
async function loadPlans() {
  try {
    ...
    if(data.usage) {
      ...
      const remainingVideos = (currentUserPlan === 'free' ? 1 : currentUserPlan === 'pro' ? 10 : 100) - (data.usage.videos || 0);
      document.getElementById('remaining-badge').textContent = `💬 ${remainingMessages} msgs · 🎨 ${remainingImages} images · 🎬 ${remainingVideos} videos left today`;
    }
