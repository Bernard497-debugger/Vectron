...
def admin_panel():
    ...
    subs_rows = "".join([f"""
        ...
        </tr>""" for uid, s in SUBSCRIPTIONS.items()])

    all_payments = {**PENDING_PAYMENTS, **VERIFIED_PAYMENTS}
    payments_rows = "".join([f"""
        <tr>
            ...
            <td>{p.get('country','—')}</td>
            <td><span style="color:{'#4ade80' if p['status']=='verified' else '#fbbf24'}">{p['status']}</span></td>
            <td>{str(p.get('created_at',''))[:10]}</td>
            <td>{f'<a href="/admin/receipt/{p["receipt_filename"]}?key={key}" target="_blank" style="color:#3b82f6;">View</a>' if p.get('receipt_filename') else '—'}</td>
        </tr>""" for pid, p in all_payments.items()])
    ...
    <body>
        ...
        <table>
            ...
            <tbody>{subs_rows or '<tr><td colspan="8" class="empty">No subscriptions yet</td></tr>'}</tbody>
        </table>
        </div>
        ...
        <table>
            ...
            <tbody>{payments_rows or '<tr><td colspan="8" class="empty">No payments yet</td></tr>'}</tbody>
        </table>
        </div>
...
def check_rate_limit():
    ...
    if usage.get("videos", 0) >= video_limit and video_limit > 0:
        return False, f"You've reached your daily video limit ({video_limit} videos)."
...
def generate_video_wan26(prompt, duration=5, resolution="512x512"):
    """Generate a video using Alibaba Wan-2.6 model (Pro only)"""
    if not OPENROUTER_API_KEY:
        raise ValueError("OPENROUTER_API_KEY not set")
    ...
    try:
        ...
        if video_url:
            ...
            try:
                ...
                return {"success": True, "video_url": f"/video/{video_filename}", "prompt": prompt}
            except:
                return {"success": True, "video_url": video_url, "prompt": prompt}
...
def serve_video(filename):
    user_plan = get_user_plan()
    if user_plan == "free":
        return jsonify({"error": "Video access is only available for Pro and Enterprise plans"}), 403
...
def initiate_payment():
    ...
    PENDING_PAYMENTS[payment_id] = {
        "user_id": user_id, "plan_id": plan_id, "amount_usd": amount_usd,
    ...
    🎬 **Pro Plan unlocks:** Video Generation with Alibaba Wan-2.6!
    """
    
    return jsonify({
        "success": True, "payment_id": payment_id, "amount_usd": amount_usd,
        "amount_local": amount_local, "currency": currency,
...
def upload_receipt():
    ...
    SUBSCRIPTIONS[user_id] = {
        "plan": plan_id, "status": "active", "started_at": datetime.now(),
        "expires_at": datetime.now() + timedelta(days=30),
        "payment_method": "orange_money", "receipt_filename": filename,
        "country": payment["country"]
    }
    ...
    return jsonify({
        "success": True,
        "message": f"Receipt uploaded! Your {SUBSCRIPTION_PLANS[plan_id]['name']} plan is now active. 🎬 Video generation unlocked!",
    })
...
@app.route("/generate-video", methods=["POST"])
def generate_video_route():
    """Generate a video - PRO ONLY"""
    user_plan = get_user_plan()
    
    if user_plan == "free":
        return jsonify({
            "error": "PRO_FEATURE_LOCKED",
            "message": "🎬 Video Generation is a Pro feature! Upgrade to unlock Alibaba Wan-2.6 video generation."
        }), 403
    ...
    try:
        video_result = generate_video_wan26(prompt)
        ...
        if video_result.get("success"):
            ...
            today = datetime.now().date().isoformat()
            usage = session.get("usage", {}).get(today, {"videos": 0})
            video_limit = 10 if user_plan == "pro" else 100
            remaining_videos = video_limit - usage.get("videos", 0)
            
            return jsonify({
                "success": True, "video_url": video_result.get("video_url"),
                "prompt": prompt, "remaining_videos": remaining_videos
            })
        else:
            return jsonify({"success": False, "error": video_result.get("message", "Video generation failed")})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def tool():
    ...
    if tool_name == "calculate":
        ...
        if user_plan == "free":
            return jsonify({
                "result": "## 🔒 Video Generation is a Pro Feature!\n\nUpgrade to Pro for just **$12/month** to unlock:\n\n✅ AI Video Generation with **Alibaba Wan-2.6**\n✅ 10 videos per day\n✅ Priority processing\n✅ Longer videos (up to 10 seconds)\n✅ Cinematic 4K quality\n\n📱 **Pay with Orange Money** - Instant activation!\n\n[Click here to upgrade](javascript:openUpgradeModal())",
                "type": "text", "requires_upgrade": True
            })
        ...
        try:
            ...
            if video_result.get("success"):
                update_usage(videos_count=1)
                return jsonify({
                    "result": f"🎬 **Video Generated with Alibaba Wan-2.6!**\n\n**Prompt:** {args}\n\n<video controls width='100%'><source src='{video_result.get('video_url')}' type='video/mp4'></video>\n\n[📥 Download Video]({video_result.get('video_url')})",
                    "video_url": video_result.get("video_url"), "type": "video"
                })
            else:
                return jsonify({"result": f"Error: {video_result.get('message', 'Failed to generate video')}", "type": "text"})
        except Exception as e:
...
# ─── HTML WITH ORIGINAL VECTRON UI + LOCKED VIDEO BUTTON ─────────────────────
...
<meta name="viewport" content="width=device-width, initial-scale=1.0, viewport-fit=cover">
<meta name="theme-color" content="#0a0a0b">
<meta name="description" content="Vectron — Powerful AI Agent">
<title>Vectron AI</title>
<link rel="preconnect" href="https://fonts.googleapis.com">
<link href="https://fonts.googleapis.com/css2?family=Sora:wght@300;400;500;600&family=JetBrains+Mono:wght@400;500&display=swap" rel="stylesheet">
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/highlight.js/11.9.0/styles/github-dark.min.css">
...
.mobile-nav-btn .nav-icon{font-size:20px;}
.mobile-nav-btn.locked{opacity:0.6;}
...
.sheet-tool-btn{
  ...
  color:var(--text);font-family:'Sora',sans-serif;font-size:13px;cursor:pointer;
}
.sheet-tool-btn.locked{opacity:0.6;}
.sheet-model-select{
  width:100%;background:var(--surface2);border:1px solid var(--border);border-radius:10px;
  color:var(--text);font-family:'Sora',sans-serif;font-size:13px;padding:12px 14px;outline:none;
}
...
@media(max-width:640px){
  ...
  .mobile-nav{display:flex;position:fixed;bottom:0;left:0;right:0;
    ...
    padding:8px 16px calc(8px + env(safe-area-inset-bottom));justify-content:space-around;}
...
<div class="sheet" id="sheet">
  ...
  <div id="sheet-tools-section">
    ...
    <div class="sheet-tools">
      ...
      <button class="sheet-tool-btn" id="mobile-video-btn" onclick="openVideoUpgrade()">🎬 Generate Video <span style="margin-left:auto;font-size:10px;color:var(--orange);">🔒 PRO</span></button>
    </div>
  </div>
  <div id="sheet-model-section">
    <div class="sheet-title">Model</div>
...
<div class="main">
  <aside class="sidebar">
    ...
    <button class="tool-btn" onclick="openTool('system_info')"><div class="tool-icon">⚙</div> System Info</button>
    <button class="tool-btn" onclick="openTool('image')"><div class="tool-icon">🎨</div> Generate Image</button>
    <button class="tool-btn locked" id="video-tool-btn" onclick="openVideoUpgrade()">
      <div class="tool-icon">🎬</div> Generate Video 
...
<div class="mobile-nav">
  <div class="mobile-nav-btns" style="display:flex;justify-content:space-around;">
    <button class="mobile-nav-btn" onclick="openSheet('tools')"><span class="nav-icon">⚙</span><span>Tools</span></button>
    <button class="mobile-nav-btn" onclick="resetChat()"><span class="nav-icon">↺</span><span>New</span></button>
    <button class="mobile-nav-btn" onclick="openSheet('model')"><span class="nav-icon">🤖</span><span>Model</span></button>
    <button class="mobile-nav-btn" onclick="openUpgradeModal()"><span class="nav-icon">📱</span><span>Upgrade</span></button>
...
let paymentStep = "select";
let countriesData = {};

// Function to handle video upgrade click
function openVideoUpgrade() {
  openUpgradeModal();
}
...
function renderMarkdown(text){
  ...
  w.querySelectorAll('pre').forEach((pre) => {
    ...
    block.innerHTML = `
      ...
      <pre><code class="language-${lang}">${escapeHtml(rawCode)}</code></pre>
    `;
    pre.replaceWith(block);
  });
